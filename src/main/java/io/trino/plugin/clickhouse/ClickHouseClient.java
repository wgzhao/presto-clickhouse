 ...
 * and reading the source code in depth, but the problem resided in my head like a ghost.
 * I once suspected that it was the problem of ClickHouse jdbc driver , and tried to replace another
 * driver package during the period, but the problem remained.
 * No matter how I configure it, the name of the catalog will always become the default database of
 * clickhouse (default), which will cause all the values obtained by `connection.getCatalog()` are not fought for.
 * <p>
 * In the end, I had to override all the methods where catalog was wrong
 */
    ...
    public Optional<ColumnMapping> toColumnMapping(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        switch (typeHandle.getJdbcType()) {
            ...
            case Types.TIMESTAMP:
                ...
                return Optional.of(timestampColumnMapping(TimestampType.TIMESTAMP_MILLIS));

            case Types.OTHER:
                // ClickHouse-special data type
                if (jdbcTypeName.startsWith("FixedString")) {
                    return Optional.of(charColumnMapping(CharType.createCharType(typeHandle.getRequiredColumnSize())));
                }
                else {
                    return Optional.of(varcharColumnMapping(createUnboundedVarcharType()));
                }
